#!/usr/bin/env python3
#Depth Anything V3 for DEM + nDSM regression (with stats-based init)
"""
...
class DA3DemNdsmModel(nn.Module):
    ...
    def __init__(
        ...
        if not train_backbone:
            print("[DA3DemNdsm] Freezing DA3 network (training adapter/task heads only)")
            for p in self.net.parameters():
                p.requires_grad = False

            # Some DA3 variants expose a dedicated depth head. Allow opting into it explicitly.
            if train_da3_head and hasattr(self.net, "head"):
                print("[DA3DemNdsm] Unfreezing DA3 depth head")
...
def train_epoch(
    ...
    for batch in pbar:
        ...
        dem_mse = masked_mse(p_dem32, dem32, m32)
        ndsm_mse = masked_mse(p_ndsm32, ndsm32, m32)
        ...
        if not torch.isfinite(loss):
            ...
            if is_master:
                ...
                print(
                    "p_dem_finite, p_ndsm_finite:",
                    float(torch.isfinite(p_dem32).float().mean()),
                    float(torch.isfinite(p_ndsm32).float().mean()),
                )
            optimizer.zero_grad(set_to_none=True)
            continue
        ...
        totals["dem_mse"] += dem_mse.item()
        totals["ndsm_mse"] += ndsm_mse.item()
        valid_steps += 1.0
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        t = torch.tensor(
            [totals[k] for k in keys] + [valid_steps, skipped_steps],
            device=device,
            dtype=torch.float64,
        )
        ...
        valid_steps = float(t[-2].item())
        skipped_steps = float(t[-1].item())
    else:
        valid_steps = float(valid_steps)
        skipped_steps = float(skipped_steps)

    if is_master and skipped_steps > 0:
        print(f"[WARN] Skipped {int(skipped_steps)} training batches due to NaN/Inf loss")

    n = max(1.0, valid_steps)
...
def validate(
    ...
    for batch in tqdm(loader, desc="Validation", disable=(not is_master)):
        x = batch["image"].to(device, non_blocking=True)
        dem = batch["dem"].to(device, non_blocking=True)
        ndsm = batch["ndsm"].to(device, non_blocking=True)
        m = batch["mask"].to(device, non_blocking=True)
        ...
        if not torch.isfinite(loss):
            skipped_steps += 1.0
            if is_master:
                print("[BAD][VAL] loss NaN/Inf, skip batch:", batch["path"][0])
                print("dem_h, dem_g, nd_h, nd_g:", float(dem_h), float(dem_g), float(nd_h), float(nd_g))
                print(
                    "p_dem_finite, p_ndsm_finite:",
                    float(torch.isfinite(p_dem32).float().mean()),
    ...
    if is_master and skipped_steps > 0:
        print(f"[WARN] Skipped {int(skipped_steps)} validation batches due to NaN/Inf loss")
...
def setup_distributed(args):
    ...
    if local_rank >= ndev:
        raise RuntimeError(
            f"Invalid local_rank={local_rank} for visible cuda count={ndev}. "
...
def main():
    ...
    ap.add_argument("--train_da3_head", action="store_true")
    ...
    ap.add_argument("--wandb_project", type=str, default=os.environ.get("WANDB_PROJECT", "da3-dem-ndsm"))
    ap.add_argument("--wandb_entity", type=str, default=os.environ.get("WANDB_ENTITY", None))
    ap.add_argument("--wandb_run_name", type=str, default=None)
    ap.add_argument("--wandb_tags", type=str, default="ddp,dem,ndsm")
    ...
    device = f"cuda:{local_rank}" if distributed else ("cuda" if torch.cuda.is_available() else "cpu")
    ...
    model = DA3DemNdsmModel(
        ...
        train_da3_head=args.train_da3_head,
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if is_master:
            ...
            if fixed_indices:
                visualize_fixed_samples(model_ref, val_ds, fixed_indices, device, epoch, vis_dir)
            ...
            if args.use_wandb:
                wandb.log({
                    ...
                    "train/loss": tr["loss"],
                    "train/dem_mse": tr["dem_mse"],
                    "train/ndsm_mse": tr["ndsm_mse"],
                    "val/loss": va["loss"],
                    "val/dem_mse": va["dem_mse"],
                    "val/ndsm_mse": va["ndsm_mse"],
                }, step=epoch)
